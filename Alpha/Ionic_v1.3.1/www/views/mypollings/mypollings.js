...
angular.module('App').controller('myPollingsController', function ($scope, $ionicModal, $state ,$cordovaOauth, $localStorage, $location,$http,$ionicPopup, $firebaseObject, $firebaseArray,$firebaseUtils, Auth, FURL, Utils,mypollkey) {
  ...
  var checkDate = new Date(now.getFullYear(),pad(now.getMonth()),pad(now.getDate()),pad(now.getHours()),pad(now.getMinutes()),0);
  $scope.date2 = Number(checkDate.getFullYear()+""+pad(Number(checkDate.getMonth()+1))+""+pad(checkDate.getDate())+""+pad(checkDate.getHours())+""+pad(checkDate.getMinutes()));
  console.log("ini date 2"+$scope.date2);
  ...
  //ini gw edit tambahin distribute + analyze
    ...
    $scope.openDistribute = function(){
      ...
      $scope.currdate =  new Date(now.getFullYear(),pad(now.getMonth()),pad(now.getDate()),pad(now.getHours()),pad(now.getMinutes()),0);
      var deadlineObj = new $firebaseObject($scope.polingref.child('deadline'));
      deadlineObj.$value = Number($scope.currdate.getFullYear()+""+pad(Number($scope.currdate.getMonth()+1))+""+pad($scope.currdate.getDate())+""+pad($scope.currdate.getHours())+""+pad($scope.currdate.getMinutes()));
      console.log(deadlineObj.$value);
      deadlineObj.$save();
  ...
  $scope.changeDate= function(date){
    ...
    var deadlineObj = new $firebaseObject($scope.polingref.child('deadline'));
    deadlineObj.$value = Number(date.getFullYear()+""+pad(Number(date.getMonth()+1))+""+pad(date.getDate())+""+pad(date.getHours())+""+pad(date.getMinutes()));
    deadlineObj.$save();
  };
